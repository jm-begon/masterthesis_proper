...
#        """
        ...
        for box in boxes:
            newCol, newRow, originCol, originRow, destCol, destRow = box
            aggregArray[newRow, newCol] = self._agg_func(
                array2D[originRow:destRow, originCol:destCol])

        return aggregArray

    def aggregate(self, npArray):
        """
        ...
        """
        if len(npArray.shape) < 2 or len(npArray.shape) > 3:
            raise ValueError

        if len(npArray.shape) == 2:
            #2D case
            return self._do_aggregate(npArray)

        #3D case
        layers = []
        for i in range(npArray.shape[2]):
            layers.append(self._do_aggregate(npArray[:, :, i]))
